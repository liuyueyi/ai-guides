import{_ as c,V as l,W as i,$ as a,Y as n,Z as s,a0 as p,F as e}from"./framework-a99a4a1a.js";const u="/ai-guides/imgs/column/springai/D03-01.webp",r="/ai-guides/imgs/column/springai/D03-02.webp",k="/ai-guides/imgs/column/springai/D03-03.webp",d="/ai-guides/imgs/column/springai/D03-04.webp",v="/ai-guides/imgs/column/springai/D03-05.webp",m="/ai-guides/imgs/column/springai/D03-06.png",b="/ai-guides/imgs/column/springai/D03-07.png",g="/ai-guides/imgs/column/springai/D03-08.png",y={},f=p('<p>发票的OCR识别对于现今而言，可以说比较成熟了；今天我们来从0到1实现一个基于大模型的发票提取智能体，也基于此看看大模型开发和传统的开发之间，有什么区别</p><h2 id="一、系统架构设计" tabindex="-1"><a class="header-anchor" href="#一、系统架构设计" aria-hidden="true">#</a> 一、系统架构设计</h2><h3 id="_1-1-设计目标" tabindex="-1"><a class="header-anchor" href="#_1-1-设计目标" aria-hidden="true">#</a> 1.1 设计目标</h3><p>大模型发票信息提取方案，主要基于SpringAI+多模态大模型，实现零配置、高泛化、端到端的发票智能提取系统，核心优势：</p><ul><li>无需模板：直接理解任意版式发票</li><li>语义理解：理解发票内容而非单纯OCR</li><li>结构化输出：直接生成可入库的数据结构</li></ul><h3 id="_1-2-系统整体架构" tabindex="-1"><a class="header-anchor" href="#_1-2-系统整体架构" aria-hidden="true">#</a> 1.2 系统整体架构</h3><p>对应的系统架构设计如下</p><figure><img src="'+u+'" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>从整体的分层结构来看，一个完整的发票提取包含：</p><ul><li>接入层：负责接收外部请求、做初步流量治理 <ul><li>定位：可以理解为全局网关，承接外部流量并过滤非法请求</li></ul></li><li>应用层：做发票的多模态解析（比如识别票据图片、解析文本） + 数据标准化（统一发票信息的格式）； <ul><li>定位： “业务中枢”，串联下游服务、实现发票提取的核心流程</li></ul></li><li>服务层：把具体技术能力封装为独立服务，供应用层调用 <ul><li>定位：能力组件库，解耦业务逻辑与技术实现。</li></ul></li><li>存储层：按数据特性分两类存储：缓存存储（Redis）存高频访问的发票信息，提升查询效率 + 持久化存储（PostgreSQL）存全量发票数据，做长期归档。 <ul><li>定位：数据仓库，保障数据的高效访问与长期留存。</li></ul></li><li>运维层：监控与审计 <ul><li>定位：运维保障，确保全流程可观测、问题可追溯</li></ul></li></ul>',10),q=p('<h3 id="_1-3-业务流程" tabindex="-1"><a class="header-anchor" href="#_1-3-业务流程" aria-hidden="true">#</a> 1.3 业务流程</h3><p>基于上面的系统架构，我们可以串一下发票信息提取的全流程</p><ul><li>用户上传发票</li><li>后端应用接受请求，做一些基本过滤</li><li>应用层处理发票识别逻辑</li><li>服务层与大模型交互，提取识别结果，并存储相关的数据</li></ul><figure><img src="'+r+`" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>graph TD
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="三、项目搭建" tabindex="-1"><a class="header-anchor" href="#三、项目搭建" aria-hidden="true">#</a> 三、项目搭建</h2>`,24),h={href:"https://hhui.top/tutorial/spring/springai/%E5%9F%BA%E7%A1%80%E7%AF%87/01.%E5%88%9B%E5%BB%BA%E4%B8%80%E4%B8%AASpringAI-Demo%E5%B7%A5%E7%A8%8B.html",target:"_blank",rel:"noopener noreferrer"},w=p(`<p>在下面的实现过程中，我们使用智谱的免费大模型作为我们的实际载体；若希望使用其他的模型的小伙伴，也可以直接替换（SpringAI对不同厂商的大模型集成得相当可以，切换成本较低）</p><h3 id="_3-1-基础环境" tabindex="-1"><a class="header-anchor" href="#_3-1-基础环境" aria-hidden="true">#</a> 3.1 基础环境</h3><p>我们使用的SpringAI的版本为最新的 <code>1.1.2</code> ，此外直接使用 zhipu 的starter来作为大模型的交互客户端</p><div class="language-xml line-numbers-mode" data-ext="xml"><pre class="language-xml"><code><span class="token comment">&lt;!-- pom.xml 关键依赖 --&gt;</span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="四、核心实现代码" tabindex="-1"><a class="header-anchor" href="#四、核心实现代码" aria-hidden="true">#</a> 四、核心实现代码</h2><h3 id="_4-1-提示词管理" tabindex="-1"><a class="header-anchor" href="#_4-1-提示词管理" aria-hidden="true">#</a> 4.1 提示词管理</h3>`,15),x={href:"https://mp.weixin.qq.com/s/96rHyp_gBUgmA2dhSbzNww",target:"_blank",rel:"noopener noreferrer"},I=n("code",null,"resources/prompt",-1),S=n("code",null,"xxx.pt",-1),P=p(`<p>一个可用于大模型提取发票的提示词如下</p><div class="language-pt line-numbers-mode" data-ext="pt"><pre class="language-pt"><code>你是一个专业的发票信息提取专家。请从用户提供的发票图片中提取完整的结构化信息。
