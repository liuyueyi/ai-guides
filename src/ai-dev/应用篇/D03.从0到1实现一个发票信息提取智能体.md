...
flowchart TB
    %% 全局样式：专业无衬线字体，统一圆角和边框
    classDef base   font-size:12px, rounded:8, stroke-width:1.5
    
    %% 抽象层级定义
    subgraph SA["👥 接入层 (Access Layer)"]
        direction TB
        AL1[多端接入网关<br/>API/SDK/批量上传]
        AL2[请求校验&路由<br/>鉴权/限流/降级]
    end
    
    
    subgraph SB["🔧 应用层 (Application Layer)"]
        direction TB
        AP1[核心处理中心<br/>多模态解析/数据标准化]
        AP2[能力扩展层<br/>Function Call/第三方集成]
    end
    
    subgraph SC["🛠️ 服务层 (Service Layer)"]
        direction TB
        SL1[AI模型服务<br/>主模型/兜底OCR]
        SL2[数据处理服务<br/>校验/缓存/格式转换]
    end
    
    subgraph SD["💾 存储层 (Storage Layer)"]
        direction TB
        ST1[缓存存储<br/>Redis]
        ST2[持久化存储<br/>PostgreSQL]
    end
    
    subgraph SE["🔍 运维层 (Ops Layer)"]
        direction TB
        OP1[监控告警<br/>指标/链路追踪]
        OP2[日志审计<br/>操作留痕/问题排查]
    end
...
![来源于百度](https://5b0988e595225.cdn.sohucs.com/images/20191017/c0cf274fec0141439a636ab0c353752a.jpeg)
