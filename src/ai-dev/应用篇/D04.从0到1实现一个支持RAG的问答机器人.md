...
![](/imgs/column/springai/D04-1.png)
...
flowchart TD
    ...
    %% 第一部分：上半区 - RAG 知识库构建阶段（数据预处理+向量入库）
    subgraph RAG["🔧 RAG 知识库构建阶段（离线）"]
        style RAG fill:#e6f4ff,stroke:#1976d2,stroke-width:2,opacity:0.6
        direction TB
        A[原始文档集<br/>【PDF/Word/网页等】]:::dataNode 
        B[文档预处理<br/>【清洗/去噪/格式标准化】]:::processNode
        C[智能分块<br/>【按语义/长度拆分，避免信息割裂】]:::processNode
        D[元数据提取<br/>【标题/作者/页码/关键词】]:::processNode
        E[构建可检索单元<br/>【分块+元数据组合】]:::processNode
        F[统一嵌入模型<br/>【将文本转为768/1024维向量】]:::processNode
        G[文本向量化<br/>【语义编码，保留上下文信息】]:::processNode
        H[向量数据库<br/>【存储向量，支持相似性检索】]:::dbNode
        ...
        %% 关键说明注释
        note1[注：分块是RAG效果的核心，过细丢失上下文，过粗检索精度低]:::noteText
        C -.-> note1
        note2[注：嵌入模型需与LLM适配，常用BGE/Text-Embedding-ADA]:::noteText
        F -.-> note2
    end
    


    %% 第二部分：下半区 - RAG 问答推理阶段（在线交互）
    subgraph SB["💬 RAG 问答推理阶段（在线）"]
        style SB fill:#e8f5e8, stroke:#2e7d32, color:#005005, opacity:0.6
        direction TB
        I[用户查询<br/>【自然语言问题】]:::queryNode
        J{查询类型判断<br/>【智能问答/基础搜索】}:::processNode
        K[查询增强策略<br/>【提升检索召回率】]:::processNode
        L[生成假设性答案（HyDE）<br/>【用假答案提升语义匹配】]:::processNode
        M[生成多个子查询（MQE）<br/>【拆分复杂问题】]:::processNode
        N[查询向量化<br/>【与知识库同模型编码】]:::processNode
        O[向量相似度搜索<br/>【Top-N 粗排】]:::processNode
        P[候选文档重排<br/>【Cross-BERT精排，提升相关性】]:::processNode
        Q[Top-K 精准文档片段<br/>【仅保留高相关内容】]:::processNode
        %% 流程连线
        I --> J
        J -->|智能问答| K
        J -->|基础搜索| N
        ...
        subgraph LLM[🤖生成与输出]
            ...
            direction TB
            R[智能上下文构建<br/>【问题+文档+提示词组合】]:::processNode
            S[HelloAgents LLM 生成<br/>【基于上下文生成答案】]:::llmNode
            T[答案后处理与校验<br/>【格式规整/事实核查】]:::processNode
            U[返回最终答案<br/>【带引用/来源】]:::queryNode
            R --> S
            S --> T
            T --> U
        end
        ...
        %% 关键说明注释
        note3[注：查询增强可使召回率提升10%-30%]:::noteText
        K -.-> note3q
        note4[注：重排是解决“检索到不相关内容”的核心手段]:::noteText
        P -.-> note4
    end
```
...
![](/imgs/column/springai/D04-2.png)

```mermaid
sequenceDiagram

    %% 角色定义：加图标+配色，区分不同模块类型
    participant U as 🧑‍💻 用户
    participant F as 🖥️ 前端界面
    participant API as 🔌 API控制器
    participant Service as 🧠 问答服务(核心)
    participant Chunker as ✂️ 文档分块器
    participant Quantizer as 📊 文档量化器(Embedding)
    participant VectorStore as 🗂️ 向量存储(Qdrant)
    participant LLM as 🤖 大语言模型

    %% 第一阶段：文档上传与向量库构建（标注阶段标题）
    note over U,LLM: 📤 阶段1：文档上传 & 向量库构建
    U->>+F: 上传本地文档(PDF/Word等)
    F->>+API: POST /api/chat/{chatId} <br/>携带文件二进制数据
    API->>+Service: 调用ask()方法（触发文档处理流程）
    Service->>Service: processFiles() <br/>【文件解析/格式校验】
    Service->>+Chunker: 分块文档 <br/>【按语义拆分，避免信息割裂】
    Chunker-->>-Service: 返回文档块列表（含元数据）
    Service->>+Quantizer: 向量化文档块 <br/>【转为128维向量，语义编码】
    Quantizer-->>-Service: 返回文档块向量表示【数组】
    Service->>+VectorStore: 添加文档向量到向量库
    VectorStore-->>-Service: 确认添加成功 <br/>（返回入库ID/数量）
    Service-->>-API: 文档处理完成响应
    API-->>-F: 返回处理结果（成功/失败）
    F-->>U: 提示"文档上传完成，可开始提问"

    %% 分隔线：视觉区分两个阶段

    %% 第二阶段：用户提问与答案生成（标注阶段标题）
    note over U,LLM: ❓ 阶段2：用户提问 & 答案生成
    U->>+F: 输入自然语言问题
    F->>+API: POST /api/chat/{chatId} <br/>携带用户问题文本
    API->>+Service: 调用ask()方法（触发问答流程）
    Service->>+Quantizer: 向量化查询文本 <br/>【与文档同模型编码】
    Quantizer-->>-Service: 返回查询向量【数组】
    Service->>+VectorStore: 检索相似文档 <br/>【余弦相似度Top-K检索】
    VectorStore-->>-Service: 返回高相关文档片段
    Service->>+LLM: 构建提示词并发送请求
    Note over Service,LLM: 📝 提示词结构：<br/>1. 系统指令（答案基于文档）<br/>2. 检索到的文档片段<br/>3. 用户原始问题
    LLM-->>-Service: 流式返回生成的答案
    Service-->>-API: SSE(Server-Sent Events)流式返回答案
    API-->>-F: SSE实时推送答案片段
    F-->>U: 逐段渲染显示最终答案
```
...
![](/imgs/column/springai/D04-3.gif)
