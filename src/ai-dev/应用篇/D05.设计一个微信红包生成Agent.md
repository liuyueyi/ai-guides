---
order: 5
title: D05.从0到1实现一个微信红包封面设计Agent
tag:
  - Spring
  - SpringAI
category:
...
sequenceDiagram
    ...
    %% 2. 参与者定义（添加语义图标，优化命名展示）
    participant User as 🧑‍💻 用户
    participant Frontend as 🖥️ 前端(redpacket.html)
    participant Backend as 🔌 后端(ChatApiController)
    participant LLM as 🤖 大模型
    participant ImgGen as 🎨 图像生成服务


    %% 3. 阶段1：页面初始化
    note over User,Frontend: 📌 阶段1：页面初始化
    User->>+Frontend: 打开红包设计页面
    Frontend->>Frontend: 初始化页面，生成唯一chatId（会话标识）
    deactivate Frontend

    %% 4. 阶段2：需求提交与多轮澄清
    note over User,LLM: 📌 阶段2：需求提交 & 多轮澄清
    User->>+Frontend: 输入设计需求并发送
    Frontend->>+Backend: GET /api/chat/{chatId}?question={用户问题}<br/>【提交设计需求】
    Backend->>+LLM: 异步调用大模型（解析需求）
    LLM-->>Backend: 触发AskUserQuestionTool【需求澄清问询】
    deactivate LLM
    
    Backend->>Frontend: SSE推送问询问题【实时推送，无需等待】
    deactivate Backend
    activate Frontend
    
    Frontend->>User: 显示问询问题给用户
    User->>Frontend: 回答问题
    Frontend->>+Backend: GET /api/send/{chatId}?msg={用户答案}<br/>【提交澄清答案】
    deactivate Frontend
    activate Backend
    
    Backend->>+LLM: 将用户答案传给大模型
    %% 多轮问询循环（优化注释，明确逻辑）
    loop 多轮需求澄清【直到需求明确】
        LLM-->>Backend: 可能返回新的问询（补充需求）
        Backend->>Frontend: SSE推送新问询
        Frontend->>User: 显示新问询
        User->>Frontend: 回答新问询
        Frontend->>Backend: 发送新答案
        Backend->>LLM: 传递新答案
    end
    
    LLM-->>Backend: 返回最终设计方案（文字版）
    deactivate LLM
    
    Backend->>Frontend: SSE推送最终设计方案
    deactivate Backend
    activate Frontend
    
    Frontend->>User: 显示设计方案（文字版）
    deactivate Frontend

    %% 5. 阶段3：红包图片生成
    note over User,ImgGen: 📌 阶段3：红包图片生成
    User->>+Frontend: 点击生成图片按钮
    Frontend->>+Backend: GET /api/genImg?msg={设计方案}<br/>【触发图片生成】
    deactivate Frontend
    activate Backend
    
    Backend->>+ImgGen: 调用图像生成API【传递设计方案参数】
    ImgGen-->>Backend: 返回生成的图片(Base64格式)【含图片二进制数据】
    deactivate ImgGen
    
    Backend->>Frontend: 返回图片Base64数据
    deactivate Backend
    activate Frontend
    
    Frontend->>User: 渲染并显示生成的红包封面
    deactivate Frontend
```
